...
-- 1. Refund the coach's room fee for the corrected sessions
update person p
set credits = credits + s.room_fee_credits
from session s
where s.id in (814, 617, 817, 805, 808, 815, 818, 819)
  and p.id = s.coach_id
  and s.status = 'scheduled';

-- 2. Refund any active participants and cancel their enrolments
update person p
set credits = credits + e.credits_charged
from enrolment e
where e.session_id in (814, 617, 817, 805, 808, 815, 818, 819)
  and e.status = 'active'
...
set status = 'cancelled',
    ...
    cancelled_at = now()
where session_id in (814, 617, 817, 805, 808, 815, 818, 819)
  and status = 'active';

-- 3. Cancel the sessions themselves
update session
set status = 'cancelled'
where id in (814, 617, 817, 805, 808, 815, 818, 819)
