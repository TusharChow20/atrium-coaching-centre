...
import { hoursOfNotice, participantRefundPercent, refundAmount, seatFee } from '../credits';

const router = Router();

// a participant sees only their own bookings only
router.get('/mine', requireSession, attachRole, requireRole('participant'), async (req, res) => {
  try {
...
router.post('/sessions/:sessionId/enrol', requireSession, attachRole, requireRole('participant'), async (req, res) => {
  try {
    ...
    if (session.coach_id === personId) {
      res.status(400).json({ error: 'a coach may not enrol in their own session' });
      return;
    }

    const people = await query("select credits from person where id = $1 and kind = 'participant' and active", [personId]);
    if (people.length === 0) {
    ...
    if (people[0].credits < fee) {
      res.status(400).json({ error: 'insufficient credits to book this place' });
      return;
    }

    const rooms = await query('select capacity from room where id = $1', [session.room_id]);
    ...
    const created = await withSerializableTransaction(async (client) => {
      ...
      if (taken.rows[0].count >= capacity) {
        throw Object.assign(new Error('that session is full'), { httpStatus: 409 });
      }
      ...
      if ((conflict.rowCount ?? 0) > 0) {
        throw Object.assign(new Error('you have a conflicting commitment at that time'), { httpStatus: 409 });
      }
      ...
      try {
        ...
        if (err.code === '23505') {
          throw Object.assign(new Error('you are already booked into that session'), { httpStatus: 409 });
        }
        throw err;
      }
    });

    // TODO  notify the coach
...
router.post('/:id/cancel', requireSession, attachRole, requireRole('participant', 'admin'), async (req, res) => {
  try {
    ...
    // TODO notify the coach 
    res.json({ id, status: 'cancelled', refund_percent: percent, credits_refunded: refund });
  } catch (err) {
    console.error(err);
